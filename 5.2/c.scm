...
; 以 (assign b (reg t)) 為例子
; (assign-reg-name inst)  => b
; value-exp = (assign-value-exp inst) => ((reg t))
; taget: compound-procedure, target 是哪個 procedure 要找出來
; taget: compound-procedure, pc 是哪個 procedure 要找出來
...
; (goto (label test-b))
; dest: (label test-b)
; insts: (((test (op =) (reg b) (const 0)) . #[compound-procedure 140]) ((branch (label gcd-done)) . #[compound-procedure 141]) ((assign t (op rem) (reg a) (reg b)) . #[compound-procedure 142]) ((assign a (reg b)) . #[compound-procedure 143]) ((assign b (reg t)) . #[compound-procedure 144]) ((goto (label test-b))))
